...
Subnet (i) Two-stream feature embedding: Our model starts by using the noisy speech’s T-F spectrogram and the concurrent ultrasound Doppler spectrogram as input (Section 5). We then design a two-stream feature embedding architecture, to transform the different modalities into the same feature space, while maintaining their time-domain alignment.
